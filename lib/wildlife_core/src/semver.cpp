...
bool parse_semver(std::string_view in, SemVer& out) noexcept {
    ...
    SemVer tmp{};
    if (!parse_uint(in.substr(0, first), tmp.major)) return false;
    if (!parse_uint(in.substr(first + 1, second - first - 1), tmp.minor)) return false;
    if (!parse_uint(in.substr(second + 1), tmp.patch)) return false;
