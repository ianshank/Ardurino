...
int64_t JVal::as_int(int64_t def) const noexcept {
    if (_type == JType::Int64)
        return _i;
    if (_type == JType::Double)
        return static_cast<int64_t>(_d);
    return def;
}
...
std::size_t JVal::size() const noexcept {
    if (_type == JType::Array)
        return _arr.size();
    if (_type == JType::Object)
        return _obj.size();
    return 0;
}
...
const JVal& JVal::operator[](std::size_t idx) const noexcept {
    if (_type != JType::Array || idx >= _arr.size())
        return null_sentinel();
    return _arr[idx];
}
