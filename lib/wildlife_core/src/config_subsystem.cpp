...
static std::string get_str(const JVal& obj, const char* key, std::string_view def) noexcept {
...
void ConfigParser::parse_wifi(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.wifi.sta_timeout_s =
        static_cast<uint16_t>(get_int(s, "sta_timeout_s", out.wifi.sta_timeout_s));
}
...
void ConfigParser::parse_mqtt(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.mqtt.max_payload_kb =
        static_cast<uint16_t>(get_int(s, "max_payload_kb", out.mqtt.max_payload_kb));
    out.mqtt.keepalive_s = static_cast<uint16_t>(get_int(s, "keepalive_s", out.mqtt.keepalive_s));
}
...
void ConfigParser::parse_ota(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.ota.check_interval_h =
        static_cast<uint8_t>(get_int(s, "check_interval_h", out.ota.check_interval_h));
}
...
void ConfigParser::parse_inference(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.inference.min_score =
        static_cast<uint8_t>(get_int(s, "min_score", out.inference.min_score));
    out.inference.debounce_ms =
        static_cast<uint32_t>(get_int(s, "debounce_ms", out.inference.debounce_ms));
    out.inference.poll_interval_ms =
        static_cast<uint32_t>(get_int(s, "poll_interval_ms", out.inference.poll_interval_ms));
    out.inference.prewake_warmup_ms =
        static_cast<uint32_t>(get_int(s, "prewake_warmup_ms", out.inference.prewake_warmup_ms));
    out.inference.sscma_invoke_times =
        static_cast<uint8_t>(get_int(s, "sscma_invoke_times", out.inference.sscma_invoke_times));
    out.inference.sscma_filter =
        static_cast<uint8_t>(get_int(s, "sscma_filter", out.inference.sscma_filter));
    out.inference.sscma_encode =
        static_cast<uint8_t>(get_int(s, "sscma_encode", out.inference.sscma_encode));
...
void ConfigParser::parse_snapshot(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.snapshot.jpeg_quality =
        static_cast<uint8_t>(get_int(s, "jpeg_quality", out.snapshot.jpeg_quality));
    out.snapshot.max_kb = static_cast<uint16_t>(get_int(s, "max_kb", out.snapshot.max_kb));
...
void ConfigParser::parse_power(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.power.wake_level = static_cast<uint8_t>(get_int(s, "wake_level", out.power.wake_level));
    out.power.sleep_seconds =
        static_cast<uint32_t>(get_int(s, "sleep_seconds", out.power.sleep_seconds));
    out.power.active_seconds =
        static_cast<uint32_t>(get_int(s, "active_seconds", out.power.active_seconds));
    out.power.low_warn_mv = static_cast<int32_t>(get_int(s, "low_warn_mv", out.power.low_warn_mv));
    out.power.low_cutoff_mv =
        static_cast<int32_t>(get_int(s, "low_cutoff_mv", out.power.low_cutoff_mv));
}
...
void ConfigParser::parse_battery(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.battery.vref_mv = static_cast<int32_t>(get_int(s, "vref_mv", out.battery.vref_mv));
    out.battery.samples = static_cast<uint8_t>(get_int(s, "samples", out.battery.samples));
}
...
bool ConfigParser::parse(const JVal& doc, RuntimeConfig& out, std::string& error_msg) noexcept {
    ...
    out.schema_version =
        static_cast<int>(get_int(doc, "schema_version", RuntimeConfig::kCurrentSchemaVersion));
...
bool SchemaMigrator::migrate(JVal& doc, int target_version) noexcept {
    ...
    if (ver > target_version)
        return false; // future schema — cannot downgrade
    if (ver == target_version)
    ...
    for (int v = ver; v < target_version; ++v) {
        ...
        if (!found)
            return false; // no migration registered for this step
    }
    return true;
}
...
void ConfigParser::parse_network(const JVal& root, RuntimeConfig& out) noexcept {
    ...
    out.network.retry.base_ms =
        static_cast<uint32_t>(get_int(s, "retry_base_ms", out.network.retry.base_ms));
    out.network.retry.max_ms =
        static_cast<uint32_t>(get_int(s, "retry_max_ms", out.network.retry.max_ms));
    out.network.retry.max_attempts =
        static_cast<uint8_t>(get_int(s, "retry_max_attempts", out.network.retry.max_attempts));
    out.network.retry.jitter_pct =
        static_cast<uint8_t>(get_int(s, "retry_jitter_pct", out.network.retry.jitter_pct));
}
