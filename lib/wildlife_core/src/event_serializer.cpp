...
EventSerializer::EventSerializer(SerializerConfig cfg) noexcept
    : _cfg(std::move(cfg)) {}
...
void EventSerializer::append_meta(std::string& out,
                                    const DeviceMeta& meta) const noexcept {
    out += "{\"device_id\":\"";
...
std::string EventSerializer::serialize(const DetectionEvent& event) const noexcept {
    ...
    return serialize_with_labels(event,
        [](int32_t) -> std::string { return ""; });
}
