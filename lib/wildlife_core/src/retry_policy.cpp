...
uint32_t RetryPolicy::delay_ms(uint32_t attempt, uint32_t entropy) const noexcept {
    ...
    // offset = [0, 2*jitter_range] mapped from entropy, then shifted to [-jitter_range,
    // +jitter_range]
    if (_cfg.jitter_pct == 0)
    ...
    int64_t result = static_cast<int64_t>(base) - static_cast<int64_t>(jitter_range) +
                     static_cast<int64_t>(offset);
    if (result < 0)
        result = 0;
    if (result > static_cast<int64_t>(_cfg.max_ms))
        result = _cfg.max_ms;
