...
Publisher::Publisher(EventDebouncer  debouncer,
                     EventSerializer serializer,
                     std::string      device_id,
                     const RuntimeConfig::Mqtt& mqtt_cfg,
                     ITransport&    transport,
                     ILogger*       log) noexcept
    : _debouncer(std::move(debouncer))
...
PublishOutcome Publisher::publish(const DetectionEvent& event,
                                   const std::string& preresolved_label,
    ...
    if (!event.detections.empty()) {
        ...
        if (!_debouncer.should_emit(d.class_id, preresolved_label, event.event_ts_ms)) {
...
bool Publisher::publish_event_json(const std::string& json) noexcept {
    ...
    const bool ok = _transport->publish(topic,
                                         payload,
                                         json.size(),
                                         _mqtt_cfg.qos,
                                         _mqtt_cfg.retain);
    if (!ok && _log) {
        ...
        std::snprintf(buf, sizeof(buf),
                      "Publisher: event publish failed (topic=%s, len=%zu)",
...
bool Publisher::publish_chunks(const JpegBuffer& jpeg,
    ...
    if (chunks.empty()) {
        if (_log) _log->warn("Publisher: chunk_jpeg returned empty — skipping snapshot");
        return false;
    }
    ...
    for (const auto& chunk : chunks) {
        ...
        if (!ok) {
            if (_log) {
                char buf[80];
                std::snprintf(buf, sizeof(buf),
                              "Publisher: snapshot chunk %u/%u publish failed",
    ...
    if (_log && all_ok) {
        char buf[80];
        std::snprintf(buf, sizeof(buf),
                      "Publisher: snapshot sent (%zu chunks, %zu bytes)",
