...
JsonLabelResolver::JsonLabelResolver(ILogger* log) noexcept
    : _log(log) {}

bool JsonLabelResolver::load(IConfigStore& store,
    ...
    if (!store.read(path, raw)) {
        if (_log) {
            ...
            std::snprintf(buf, sizeof(buf),
                          "LabelResolver: cannot read '%s'", path.c_str());
            _log->error(buf);
        }
        return false;
    }
    ...
    _hash = sha256_hex(reinterpret_cast<const uint8_t*>(raw.data()),
                       raw.size());
    ...
    if (!root || root->type() != json::JType::Object) {
        if (_log) {
            char buf[128];
            std::snprintf(buf, sizeof(buf),
                          "LabelResolver: JSON parse failed for '%s'", path.c_str());
            _log->error(buf);
        }
        return false;
    }
    ...
    for (const auto& [k, v] : root->as_object_ref()) {
        if (v.type() != json::JType::Str) continue;
        // Keys are expected to be stringified integers: "0", "1", …
        char* endp = nullptr;
        long id = std::strtol(k.c_str(), &endp, 10);
        if (endp == k.c_str() || *endp != '\0') continue; // non-integer key
        _labels.emplace(static_cast<int32_t>(id),
                         std::string(v.as_str()));
    }
    ...
    if (_log) {
        char buf[64];
        std::snprintf(buf, sizeof(buf),
                      "LabelResolver: loaded %zu labels from '%s'",
