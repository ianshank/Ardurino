...
void MqttTransport::configure(std::string_view host, uint16_t port, uint16_t keepalive_s,
    ...
    const std::size_t requested_bytes =
        static_cast<std::size_t>(max_payload_kb) * wildlife::kBytesPerKb;
...
bool MqttTransport::connect(std::string_view client_id, std::string_view username,
    ...
    return _client.connect(client_id_buf.c_str(), username_buf.c_str(), password_buf.c_str());
}

bool MqttTransport::publish(std::string_view topic, const uint8_t* data, std::size_t len,
                            uint8_t qos, bool retain) noexcept {
    // PubSubClient (Arduino) supports QoS 0 publish only. Document the
    // downgrade once per process so the operator can see why a higher
    // QoS request silently went out at-most-once.
    if (qos > 0 && !_qos_warning_emitted) {
        if (_logger) {
            _logger->log(LogLevel::Warn, "MqttTransport: QoS>0 requested but PubSubClient "
                                         "publishes QoS 0 only; downgrading.");
        }
        _qos_warning_emitted = true;
    }
