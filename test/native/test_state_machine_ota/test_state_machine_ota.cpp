...
static void check(AppState from, AppEvent ev,
                  AppState expected_state, AppAction expected_action0) {
...
void test_ota_check_due_enters_updating() {
    check(AppState::Active, AppEvent::OtaCheckDue,
          AppState::Updating, AppAction::CheckOta);
}

// OTA available → apply
void test_ota_available_triggers_apply() {
    check(AppState::Updating, AppEvent::OtaAvailable,
          AppState::Updating, AppAction::ApplyOta);
}

// OTA applied → reboot
void test_ota_applied_triggers_reboot() {
    check(AppState::Updating, AppEvent::OtaApplied,
          AppState::Updating, AppAction::Reboot);
}

// OTA failed → back to Active
void test_ota_failed_returns_to_active() {
    check(AppState::Updating, AppEvent::OtaFailed,
          AppState::Active, AppAction::StartInference);
}

// Network drops during OTA
void test_ota_network_down_goes_to_error() {
    check(AppState::Updating, AppEvent::NetworkDown,
...
void test_ota_up_to_date_returns_to_active_with_mark_valid() {
    ...
    TEST_ASSERT_EQUAL_INT(static_cast<int>(AppState::Active),
                           static_cast<int>(r.new_state));
    TEST_ASSERT_EQUAL_INT(static_cast<int>(AppAction::MarkValid),
                           static_cast<int>(r.actions[0]));
...
void test_updating_config_loaded_is_unhandled() {
    ...
    TEST_ASSERT_EQUAL_INT(static_cast<int>(AppState::Updating),
                           static_cast<int>(r.new_state));
...
void test_ota_fatal_triggers_reboot() {
    check(AppState::Updating, AppEvent::FatalError,
