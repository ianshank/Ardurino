...
void test_zero_attempt_returns_base_ms() {
    RetryPolicy p{RetryConfig{.base_ms=1000,.max_ms=30000,.max_attempts=5,.jitter_pct=0}};
    TEST_ASSERT_EQUAL_UINT32(1000u, p.delay_ms(0));
}

void test_backoff_doubles_each_attempt() {
    RetryPolicy p{RetryConfig{.base_ms=500,.max_ms=100000,.max_attempts=10,.jitter_pct=0}};
...
void test_backoff_capped_at_max() {
    RetryPolicy p{RetryConfig{.base_ms=1000,.max_ms=5000,.max_attempts=10,.jitter_pct=0}};
    TEST_ASSERT_EQUAL_UINT32(5000u, p.delay_ms(10));
    TEST_ASSERT_EQUAL_UINT32(5000u, p.delay_ms(20));
}

void test_jitter_within_bounds() {
    RetryPolicy p{RetryConfig{.base_ms=1000,.max_ms=30000,.max_attempts=10,.jitter_pct=25}};
